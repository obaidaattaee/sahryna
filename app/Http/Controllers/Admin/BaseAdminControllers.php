...
class BaseAdminControllers extends Controller{
    function index(){
        ...
        $advertisements = Advertisement::with('user')
                ->with('userSubscriptions')
                ->with('contributes')
                ->where('accepted' , 0)->get();
        $advertisement_counter = Advertisement::count() ;
        $advertisement_success_counter = Advertisement::with('user')
                ->with('userSubscriptions')
                ->with('contributes')
                ->get()->map(function ($advetisement) {
            return  $advetisement->number_of_partners -  $advetisement->contributes->sum('number_of_parts') == 0 ?
                                                                $advetisement : null ;
                                                            })->filter()->all() ;
